"""
Isolate indexer_kernel's score output (acc_ptr) from topk_kernel's
selection, to determine whether the remaining bug is in the score
computation (dequantization/scoring) or in topk_kernel's selection logic.
"""
import torch
import sys
sys.path.insert(0, '/tmp')
from golden_indexer_reference import run as golden_run, dequant_fp8_kv_cache
...
# ---- Compute REFERENCE per-token scores directly (before topk selection) ----
K_all_ref = dequant_fp8_kv_cache(k_index_cache_fp8)  # [num_pages, page_size, head_dim]
...
print("Reference scores for batch 0, first 10 tokens:", final_scores_ref[:10])
print("Reference scores stats: min=%.4f max=%.4f mean=%.4f" % (final_scores_ref.min(), final_scores_ref.max(), final_scores_ref.mean()))

# ---- Compute OUR indexer_kernel's scores directly ----
...
our_scores_b0 = acc[seq_offsets[0]:seq_offsets[0]+seq_lens_list[0]]
print("\nOur kernel scores for batch 0, first 10 tokens:", our_scores_b0[:10])
print("Our scores stats: min=%.4f max=%.4f mean=%.4f" % (our_scores_b0.min(), our_scores_b0.max(), our_scores_b0.mean()))

diff = (final_scores_ref - our_scores_b0).abs()

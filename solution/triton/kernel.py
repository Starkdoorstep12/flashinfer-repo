...
def dsa_fwd_kernel_splitk_v4(
    ...
    if is_last:
        # ── Inline reduction: this program only, no extra kernel launch ──
        # v4 change: avoid the O(SPLIT_K^2) masked-sum row-extraction from
        # v3 (alpha_row = sum(where(offs_s == s, alpha_s, 0), axis=0) —
        # done SPLIT_K times, each scanning all SPLIT_K rows). Instead,
        # compute m_final/l_final the same vectorized way (cheap, O(SPLIT_K)),
        # then in the acc loop, load each split's m/l directly via a
        # scalar pointer offset and compute that split's alpha fresh
        # (O(1) per iteration, O(SPLIT_K) total instead of O(SPLIT_K^2)).
        offs_s = t1.arange(0, SPLIT_K)
        ...
        m_final_row = m_final[None, :]
        finite_mask = m_final_row != -float("inf")
        alpha_s_full = t1.where(finite_mask, t1.exp(m_split - m_final_row), 0.0)  # [SPLIT_K, BLOCK_H]
        l_final = t1.sum(alpha_s_full * l_split, axis=0)  # [BLOCK_H]
        ...
        for s in range(SPLIT_K):
            ...
            # Direct scalar-offset load of this split's m, instead of
            # extracting from the already-materialized alpha_s_full tensor.
            pm_s_ptrs = PARTIAL_M + tok_id * stride_pm_tok + s * stride_pm_split + offs_h * stride_pm_h
            m_s = t1.load(pm_s_ptrs)  # [BLOCK_H]
            alpha_row = t1.where(m_final != -float("inf"), t1.exp(m_s - m_final), 0.0)  # [BLOCK_H]
            acc_final += alpha_row[:, None] * acc_s
...
def kernel_splitk_v4(q_nope, q_pe, ckv_cache, kpe_cache, sparse_indices, sm_scale, SPLIT_K=16, BLOCK_N=None):
    """
    Same as kernel_splitk_v3, but the inline reduction loop avoids the
    O(SPLIT_K^2) masked-sum row-extraction (a Triton-indexing workaround)
    in favor of direct scalar-pointer loads per split, O(SPLIT_K) total
    instead of O(SPLIT_K^2). See docs/SPLITK_OPTIMIZATION.md.
    """
    num_tokens, num_qo_heads, head_dim_ckv = q_nope.shape
    num_pages, page_size, _ = ckv_cache.shape
    head_dim_kpe = q_pe.shape[-1]

"""
Tests whether the crash requires workload 1 specifically (the smallest,
most degenerate shape: batch=1, pages=1, seq_lens=[2]), or happens with
any two distinct shapes. Runs workload 2 -> workload 3 (skipping the
tiny workload 1) to see if the crash still occurs.
"""
...
def run_one(wl, label):
    ...
    print(f"[{label}] uuid={uuid[:8]} batch_size={batch_size} max_num_pages={max_num_pages} seq_lens={seq_lens.tolist()}")
    t0 = time.time()
    ...
    print(f"[{label}] SUCCESS in {time.time()-t0:.2f}s")

print("=== Running workload 2 (skip workload 1 entirely) ===")
run_one(workloads[1], "WL2")
print("\n=== Running workload 3 ===")
run_one(workloads[2], "WL3")
print("BOTH DONE")

"""
Reproduces measure_bucketing_benefit.py's EXACT structure (same
make_inputs/cleanup functions, same in-process subprocess.run cache-clear)
but limited to just the first 3 workloads, to test whether the crash is
tied to this specific script's structure rather than compile state.
"""
...
print("=== EXACT-STRUCTURE REPRO (subprocess cache clear + first 3 workloads) ===")
# subprocess.run removed for this test
for i, wl in enumerate(workloads):
    ...
    try:
        ...
        print(f"  [{i+1}/{len(workloads)}] batch={wl['axes']['batch_size']:3d} pages={wl['axes']['max_num_pages']:3d}  FAILED: {type(e).__name__}: {str(e)[:150]}")
        try:
            cleanup(inp)
        except Exception:
            pass

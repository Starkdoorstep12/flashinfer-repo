"""
Stress test: run workload 1 -> workload 2 back-to-back, many times, to
determine whether the illegal-memory-access crash seen during the
30-workload timing run is a real, reproducible-at-some-rate bug, or was
a one-off external factor. Does NOT refresh/reset anything between
iterations within a trial -- each trial is a fresh process invocation
(via a shell loop) to avoid masking a real bug with subprocess isolation.
"""
...
try:
    ...
    print(f"trial {trial}: FAIL - {type(e).__name__}: {str(e)[:100]}")

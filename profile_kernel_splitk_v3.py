...
from solution.triton.kernel import kernel_splitk_v3
...
for _ in range(10):
    output, lse = kernel_splitk_v3(q_nope, q_pe, ckv_cache, kpe_cache, sparse_indices, sm_scale, SPLIT_K=SPLIT_K)
...
torch.cuda.synchronize()
print(f"SPLIT_K={SPLIT_K}  Avg latency: {start.elapsed_time(end)/50:.4f} ms")
